from random import *
import sys
#imports; could be made more efficient?
gold, bankedGold = 0, 0
...
#Line 4-33: Starting variables, organised with arbitrary rules
...
#Function that decides "true" stats, as seen by the print statements
        ...
        case "Mage":
            ...
            while True:
                ...
                if whichCat in baseSpellsDict:
                    ...
                    #Adds the spell chosen into the spells dictionary so long as the input is in baseSpellsDict
...
#Dictates what slot an item takes up, following an arbitary set of rules. Stops items from being used in incorrect slots
...
#Two methods of showing what is currently equipped since text files cannot comprehend dictionaries without imports or complicated code
slot_mapping = {
    "H": "Head",
    "C": "Chest",
    "G": "Gloves",
...
#Determines level recommended to enter an area
...
#Dictates price and ALSO makes anything in this dict "illegal" in the normal shop
...
#Lines 194-228: more variables. Self Explanatory
...
create = ["teleport scroll", "protect scroll", "sprites scroll", "zombie scroll", "swift scroll", "freeze scroll",
    ...
    "lightning scroll"]
#Lines 231-246: for the Alchemist
summonStats = {
    "imp": (25, 1, 1, 2, 10,),
    "warlord": (100, 7, 9, 5, 1,),
    "dictator": (75, 3, 3, 1, 5,),
...
#Determines the power/stats of summoned units for summoners ect.
nTotHea = TotHea
#Determines what health is based on your current total health. Used for fighting without changing actual Total Health
def classChange(newClass, itemUsing, lev, stre, agi, dex, hea, per, cha, inte, TotHea, specialAbility, specialAbilityValue):
    ...
    while True:
        ...
        if q == "Yes" or q == "Y":
            ...
            print("That is not an option.")
#Changes class using items and Function classv to the class that corresponds with the item, seen in dict originClass
...
def rarities(q):
    ...
    if q in inventory:
        ...
        print("NO2")
#Dictates how strong the buff an item has and adds it to weaponModifier
#MAY BE DEFUNCT; CHECK!!!
...
def statBoosts(equipment):
    ...
    #Linked to ARMORY and equipping items
    try:
        upLevel = int(eList[len(eList)-1].replace("+",""))
        eList.pop(len(eList)-1)
        " ".join(eList)
        #If possible, replaces the + in an item's name (e.g. sword+1 removes the + between sword and 1)
    except:
        print(equipment.title()+" has no upgrades")
        upLevel = 0.5
        #If it has no + to replace, it is assumed that it has not been upgraded, so base multiplier turns to 0.5, not 0 so stat isn't 0
    finally:
        equipmentBuff = {
        ...
        #Determines what benefit each item will give
        print(equipmentBuff[" ".join(eList)])
        #Outputs all the stat benefits given by the equipment CURRENTLY equipped
        ...
        for i in range(int(len(equipmentBuff[" ".join(eList)])/2)):
            ...
            match equipmentBuff[" ".join(eList)][count]:
                ...
                case "N/A":
                    print("")
                #Changes the temporary stats to account for equipment benefits. This also keeps the true stats intact which allows...
                #...for stats to be reverted after equipment is removed. This works since after one equipment piece is removed, the...
                #...whole check is performed again.
            count += 2
            count1 += 2
...
#HERE# is for places which need further investigation, most likely because its purpose has been forgotten
